...
cc.Class({
    ...
    onBtnJrokCliked:function(){
        ...
        var onGet=function(ret){
            ...
            if(ret.errcode!==0){
                ...
                if(ret.errcode==4){
                    cc.vv.alert.show("提示","您无权限加入该俱乐部");
                    this.jreditbox.string='';
                }
            }
            else{
            ...
            if (ret.errcode !== 0) {
              ...
              //  console.log(self.cjclubroombtn.clubid );
                item.on(cc.Node.EventType.TOUCH_END,function(event){
                    ...
                    self.label5.string=this.clubid;  
                    ...
                    self.club_Win.getChildByName("clubRoomWin").getChildByName("glclubroombtn").active=true;
                    self.club_Win.getChildByName("clubRoomWin").getChildByName("quitclubroombtn").active=false;
                    ...
                    // self.club_Win.getChildByName("clubRoomWin").getChildByName("Label9").active=false;                          
                        ...
                        self.club_Win.getChildByName("clubRoomWin").getChildByName("quitclubroombtn").active=true;
                        self.club_Win.getChildByName("clubRoomWin").getChildByName("glclubroombtn").active=false;
                        //self.club_Win.getChildByName("clubRoomWin").getChildByName("glclubroombtn").active=false;  
    ...
    onBtnglclubcloseCliked:function(){
        ...
        //this.club_Win.getChildByName("clubRoomWin").getChildByName("glclub").active=false;
    },
    onBtnglclubCliked:function(){
        this.glclub_Win.active=true;
       // this.club_Win.getChildByName("clubRoomWin").getChildByName("glclub").active=true;
        var data = {
            account: cc.vv.userMgr.account,
            sign: cc.vv.userMgr.sign,
        ...
        cc.vv.http.sendRequest("/get_member_info", data, function (ret) {
            var content=self.glclub_Win.getChildByName("scrollView").getChildByName("view").getChildByName("content");
            content.removeAllChildren();
            if (ret.errcode !== 0) {
                console.log(ret.errmsg);
            }
