cc.Class({
    ...
    onLoad: function () {
        ...
        if(cc.vv.gameNetMgr.conf.type == "sjmj"  || cc.vv.gameNetMgr.conf.type == 'djmj'||cc.vv.gameNetMgr.conf.type == 'kdd'){
            this._gameover = this.node.getChildByName("game_over");
        }
        else{
            if(cc.vv.gameNetMgr.conf.type == 'bymj'){
                this._gameover = this.node.getChildByName("game_over_bymj");
            }
        ...
        for(var i = 1; i <= 4; ++i){
            ...
            viewdata.reason = sn.getChildByName('reason');
            ...
            //..
            if(cc.vv.gameNetMgr.conf.type == "sjmj" || cc.vv.gameNetMgr.conf.type == "djmj"|| cc.vv.gameNetMgr.conf.type == "kdd"){
                viewdata.gangfen = sn.getChildByName("fen").getChildByName("gangfen").getComponent(cc.Label);
                viewdata.jinfen = sn.getChildByName("fen").getChildByName("jinfen").getComponent(cc.Label);
                viewdata.hufen = sn.getChildByName("fen").getChildByName("hufen").getComponent(cc.Label);
            }
            if(cc.vv.gameNetMgr.conf.type == "kdd"){
                
                sn.getChildByName("fen").getChildByName("jinfen").active = false;
                sn.getChildByName("lable").getChildByName("jinLabel").active = false;
            }
            //..
    ...
    onGameOver(data){
        ...
        console.log("type===="+cc.vv.gameNetMgr.conf.type);
        console.log(data);
        if(cc.vv.gameNetMgr.conf.type == 'sjmj' || cc.vv.gameNetMgr.conf.type == 'djmj'|| cc.vv.gameNetMgr.conf.type == 'kdd'){
           console.log("onGameOver_SJMJ");
            this.onGameOver_SJMJ(data);
        }else{
            if(cc.vv.gameNetMgr.conf.type == 'bymj'){
                this.onGameOver_BYMJ(data);
            }
        }
    ...
    onGameOver_BYMJ(data){
        console.log("in gameover data");
        console.log(data);
        if(data.length == 0){
        ...
        for(var i = 0; i < 4; ++i){
            ...
            seatView.reason.active  =false;
            //胡牌的玩家才显示 是否清一色 根xn的字样
            var numOfGangs =userData.angangs.length + userData.diangangs.length + userData.wangangs.length;
           // var numOfGen = userData.numofgen;
            ...
            if(hued){
                ...
                //     actionArr.push("清一色");
                // }
                seatView.hu.children[o].active = true;    
            ...
            console.log("++++++++++++++++"+userData.zimo)
            if(userData.zimo){
                seatView.reason.active = true;
            }
            ...
            for(var k = 0; k < gangs.length; ++k){
                var mjid = gangs[k];
                this.initPengAndGangs(seatView,index,mjid,"wangangs");
                index++;    
            }
    ...
    onGameOver_SJMJ(data){
        ...
        for(var i = 0; i < 4; ++i){
            ...
            var hued = false;
            //胡牌的玩家才显示 是否清一色 根xn的字样
            var numOfGangs = userData.wangangs.length + userData.angangs.length + userData.diangangs.length;
           // var numOfGen = userData.numofgen;
            var actionArr = [];
            //var is7pairs = false;
            //var ischadajiao = false;
            ...
            if(userData.hufen > 0){
                seatView.hufen.string = "+" + userData.hufen;    
            }
            else{
                seatView.hufen.string = userData.hufen;
            }
            ...
            if(userData.gangfen > 0){
                seatView.gangfen.string = "+" + userData.gangfen;    
            }
            else{
                seatView.gangfen.string = userData.gangfen;
            }
            ...
            if( cc.vv.gameNetMgr.conf.type !="kdd"){
                if(userData.jinfen > 0){
                    seatView.jinfen.string = "+" + userData.jinfen;    
                }
                else{
                    seatView.jinfen.string = userData.jinfen;
                }
